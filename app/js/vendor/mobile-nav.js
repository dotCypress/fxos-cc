...
define(['angular'], function() {
   ...
   */
  angular.module('mobile-navigate').factory('$change', ['$rootScope',
  ...
  function($rootScope) {
    ...
    return function change(next, prev, transType, reverse, options, cb) {
      ...
      //Convert a preset (eg 'modal') to its array of preset classes if it
      //exists; else, just convert eg 'slide' to ['slide', 'slide'],
      // so both elements get it
      //The array layout is [nextClass, prevClass]
      var transition = transitionPresets[transType] ?
        transitionPresets[transType] :
        [transType, transType];

      //Hack for white flash: z-index stops flash, offsetWidth thing forces
      // z-index to apply
  ...
  angular.module('mobile-navigate').service('$navigate',
  ['$rootScope', '$location', function($rootScope, $location) {
    ...
    function navigate(destination, source, isBack) {
      $rootScope.$broadcast('$pageTransitionStart',
        destination, source, isBack);
     ...
     * /link1 then press back before /link1 is done, it will go listen for the
     * back
     */
    ...
    self.history=[];
    self.go = function go(path, transition, reverse, silent) {
      ...
      self.onRouteSuccess = function($event, next, last) {
        ...
        if(silent !== true){
          self.history.unshift({path: path, transition: transition});
        }
  ...
  angular.module('mobile-navigate').directive('mobileView',
  ['$rootScope', '$compile', '$controller', '$route', '$change',
  function($rootScope, $compile, $controller, $route, $change) {
    function link(scope, viewElement, attrs) {
      function insertPage(page) {
        ...
        page.element = angular.element('<div class="mb-page">' +
                                        locals.$template + '</div>');
        page.element.css({
          'display': 'none'
        });
      ...
      scope.$on('$pageTransitionStart',
                  function transitionStart($event, dest, source, reverse) {
        function transition() {
          insertPage(dest);
          var _source = (source ? source.element : null);
          var _transition = dest.transition || source.transition;

          $change(dest.element, _source, _transition,
                  reverse, null, function() {
            if (source) {
